...
class RoadEdge < ActiveRecord::Base
  ...
  def self.edge_between_intersection_coordinates(raw_coord1, raw_coord2)
    ...
    intersection1 = Intersection.where(
      latitude: parsed_coord1[:latitude],
      longitude: parsed_coord1[:longitude]
    ).first
    raise "[#{raw_coord1[:longitude]},#{raw_coord1[:latitude]}] is not an intersection" if intersection1.nil?

    intersection2 = Intersection.where(
      latitude: parsed_coord2[:latitude],
      longitude: parsed_coord2[:longitude]
    ).first
    raise "[#{raw_coord2[:longitude]},#{raw_coord2[:latitude]}] is not an intersection" if intersection2.nil?
